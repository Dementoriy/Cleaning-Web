...
public class AddressHandler : RequestHandler
{
    ...
    [Post("get-addresses-by-string")]
    public void getAddressesByString()
    {
        if (!Headers.TryGetValue("Access-Token", out var token) || !TokenWorker.CheckToken(token))
        {
            Send(new AnswerModel(false, null, 400, "incorrect request"));
        ...
        var body = Bind <stringMassModel>();

        if (body.stringMass == null)
        {
            Send(new AnswerModel(false, null, 400, "incorrect request"));
            return;
        }
        ...
        foreach (var fullAddress in body.stringMass)
        {
            Address address = ClientAddresses.GetClientAddressesByFullAddress(fullAddress).First();
            AddressModel am = new AddressModel(address.ID, address.RoomType.Type, address.RoomType.Сoefficient,
                address.AddressName, address.FullAddress, address.CurrentAddress);
            addressModels.Add(am);
        }
    ...
    [Post("get-addresses-by-full-address")]
    public void GetAddressByFullAddress()
    {
        if (!Headers.TryGetValue("Access-Token", out var token) || !TokenWorker.CheckToken(token))
        {
            Send(new AnswerModel(false, null, 400, "incorrect request"));
        ...
        var body = Bind<OneAddressModel>();

        if (body.selectedAddress == null)
        {
            Send(new AnswerModel(false, null, 400, "incorrect request"));
            return;
        }
        ...
        Address address = ClientAddresses.GetClientAddressesByFullAddress(body.selectedAddress).First();


        AddressModel addressModel = new AddressModel(address.ID, address.RoomType.Type, address.RoomType.Сoefficient,
            address.AddressName, address.FullAddress, address.CurrentAddress);

        Send(new AnswerModel(true, new { address = addressModel }, null, null));
    }
}